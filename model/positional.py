...
class RotaryPositionEmbedding(nn.Module):
    ...
    │  为什么叫"旋转"位置编码?                                         │
    │                                                                  │
    │  核心思想: 把 d_k 维向量拆成 d_k/2 个 2D 向量, 每个 2D 向量     │
    │  绕原点旋转一个角度。旋转后的向量 = 原向量"转了个方向"。        │
    │                                                                  │
    │  旋转矩阵 (2D, 角度 θ):                                          │
    │    [cosθ  -sinθ]                                                 │
    │    [sinθ   cosθ]                                                 │
    ...
    │                                                                  │
    │  为什么用旋转? 因为旋转有个美妙性质:                             │
    │    旋转后的 q 和 k 做点积, 结果只取决于 "角度差",               │
    │    也就是只取决于 "相对位置差", 不取决于绝对位置。               │
    │                                                                  │
    │  用具体数字理解 (只看一对2D):                                    │
    │                                                                  │
    │    q 在位置2, 角度 θ=1.0: 旋转了 2×1.0 = 2 弧度                 │
    │    k 在位置0, 角度 θ=1.0: 旋转了 0×1.0 = 0 弧度                 │
    │                                                                  │
    │    q·k = |q||k|·cos(角度差) = |q||k|·cos(2)                    │
    │                                                                  │
    │    如果 q 在位置5, k 在位置3: 角度差也是 2                        │
    │    q·k 的值完全一样!                                             │
    │                                                                  │
    │    → 编码的是"距离2", 不是"位置2和位置0"                       │
    │                                                                  │
    ...
    │                                                                  │
    │    高频 (θ大): 位置差1就转很多 → 区分相邻token (短程)         │
    │    低频 (θ小): 位置差1几乎没转 → 感受远距离关系 (长程)        │
    │                                                                  │
    │    就像钟表: 秒针转得快(看短时间), 时针转得慢(看长时间)         │
    │    → 32 个"钟表"同时编码不同尺度的位置关系                    │
    └──────────────────────────────────────────────────────────────────┘

    ┌──────────────────────────────────────────────────────────────────┐
    │  复数视角 (理解 _rotate_half 的关键)                             │
    │                                                                  │
    │  一个2D向量 (x, y) 可以看作复数: z = x + yi                     │
    │  乘以复数 e^(iθ) = cosθ + i·sinθ:                               │
    │                                                                  │
    │    z' = z · e^(iθ)                                              │
    │        = (x + yi)(cosθ + i·sinθ)                                │
    │        = x·cosθ - y·sinθ + i(x·sinθ + y·cosθ)                   │
    │                                                                  │
    │  实部: x·cosθ - y·sinθ   ← 对应旋转后的 x'                      │
    │  虚部: x·sinθ + y·cosθ   ← 对应旋转后的 y'                      │
    │                                                                  │
    │  所以 "复数乘法" 就是 "2D 旋转"!                                │
    │                                                                  │
    │  现在推广到64维: 把它看作32个复数并排:                          │
    │    [z₀, z₁, ..., z₃₁] = [(x₀,y₀), (x₂,y₂), ..., (x₆₂,y₆₂)]   │
    │                                                                  │
    │  对位置 m, 每个复数乘以 e^(i·m·θⱼ):                            │
    │    zⱼ' = zⱼ · e^(i·m·θⱼ)                                        │
    │                                                                  │
    │  这就是 RoPE 的数学本质!                                         │
    └──────────────────────────────────────────────────────────────────┘
    ...
    def _build_cache(self, max_len: int) -> None:
        ...
        │    (q₀,q₁) 这一对用 cos[0] 和 sin[0],                      │
        │    (q₂,q₃) 这一对也用 cos[0] 和 sin[0] — 和前一对一样     │
    ...
    def _rotate_half(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ┌──────────────────────────────────────────────────────────────┐
        │  核心思想: 把 d_k 维向量看成 d_k/2 个复数并排                 │
        │                                                              │
        │  数据布局 (关键!):                                            │
        │    x = [a₀, a₁, ..., a_{d/2-1},  b₀, b₁, ..., b_{d/2-1}]   │
        │         ↑前半: 各对的第一个分量    ↑后半: 各对的第二个分量   │
        │    其中 (aᵢ, bᵢ) 是一对要旋转的 2D 向量                     │
        │                                                              │
        │  例 d_k=4: x = [a, c, b, d]                                  │
        │    (a,b) 是一对, (c,d) 是另一对                              │
        │                                                              │
        │  rotate_half(x) = [-b₀,-b₁,..., a₀,a₁,...]                 │
        │    即: 把后半搬到前面并取负                                   │
        │                                                              │
        │  例 d_k=4: [-b, -d, a, c]                                    │
        │                                                              │
        │  ┌─ 为什么这样就能旋转? ─────────────────────────────────┐ │
        │  │                                                        │ │
        │  │  我们要算: x * cos + rotate_half(x) * sin              │ │
        │  │                                                        │ │
        │  │  x * cos = [a·cos, c·cos, b·cos, d·cos]               │ │
        │  │  rotate_half(x) * sin = [-b·sin, -d·sin, a·sin, c·sin]│ │
        │  │                                                        │ │
        │  │  相加结果:                                             │ │
        │  │    [0] = a·cos - b·sin  ← (a,b) 旋转后第1个分量      │ │
        │  │    [1] = c·cos - d·sin  ← (c,d) 旋转后第1个分量      │ │
        │  │    [2] = b·cos + a·sin  ← (a,b) 旋转后第2个分量      │ │
        │  │    [3] = d·cos + c·sin  ← (c,d) 旋转后第2个分量      │ │
        │  │                                                        │ │
        │  │  每对都完美符合 2D 旋转公式!                           │ │
        │  │    x' = x·cosθ - y·sinθ                                │ │
        │  │    y' = x·sinθ + y·cosθ                                │ │
        ...
        │                                                              │
        │  为什么数据要这样排?                                          │
        │    前半放所有"第一个分量"，后半放所有"第二个分量"           │
        │    这样 rotate_half 只需整体搬运，无需逐对操作              │
        │    → 纯向量化，GPU 并行计算极快                              │
        └──────────────────────────────────────────────────────────────┘
        """
        half = x.shape[-1] // 2
